(()=>{
  ...
  let root,active='today',organizationStatus='',mode=localStorage.getItem('still-workbench-mode')||'manager',selected=new Set(),state={today:null,daily:null,tasks:[],approvals:[],customers:[],products:[],branches:[],buyerData:false};
  ...
  function shell(event){
    const nextStatus=event?.detail?.organization?.status;
    if(nextStatus)organizationStatus=nextStatus;
    const existing=$('#businessWorkbenchV72');
    if(existing){root=existing;const parking=$('#cos120ToolParking');if(parking&&root.parentNode!==parking)parking.append(root);return load()}
  ...
  async function load(){
    ...
    try{
      const buyerData=organizationStatus==='verified';
      const [todayData,daily,tasks,approvals,customers,products,branches]=await Promise.all([buyerData?api('/api/v1/business/today'):Promise.resolve({summary:{},cases:[],mostMissing:[]}),api('/api/v1/business/daily-summary'),api('/api/v1/business/tasks'),api('/api/v1/business/approvals'),buyerData?api('/api/v1/business/customers'):Promise.resolve({customers:[]}),buyerData?api('/api/v1/business/products'):Promise.resolve({products:[]}),buyerData?api('/api/v1/business/branch-insights'):Promise.resolve({branches:[]})]);
      state={today:todayData,daily,tasks:tasks.tasks||[],approvals:approvals.approvals||[],customers:customers.customers||[],products:products.products||[],branches:branches.branches||[],buyerData};selected.clear();renderBrief();render(active);const updated=$('[data-updated]',root);if(updated)updated.textContent=t('Ažurirano upravo sada','Updated just now');
    }catch(e){
      if(e.status===401){root.remove();return}
  ...
  function renderBrief(){
    ...
    if(!state.buyerData)$('[data-brief]',root).insertAdjacentHTML('afterbegin',`<div class="bw72-access-note"><b>${t('Privatni operativni alati su aktivni.','Private operational tools are active.')}</b><span>${t('Podaci kupaca i slučajevi postat će dostupni nakon verifikacije tvrtke.','Customer data and cases become available after company verification.')}</span></div>`);
  ...
  function render(tab){
    ...
    else if(['customers','products','branches'].includes(tab)&&!state.buyerData)v.innerHTML=protectedBuyerData();
  ...
  function protectedBuyerData(){return `<section class="bw72-protected"><span>${t('ZAŠTIĆENI PODACI KUPACA','PROTECTED CUSTOMER DATA')}</span><h3>${t('Dovrši verifikaciju tvrtke','Complete company verification')}</h3><p>${t('Zadaci, odobrenja i interni rad ostaju dostupni. Identiteti kupaca, proizvodi i slučajevi otvaraju se tek nakon verifikacije.','Tasks, approvals and internal work remain available. Customer identities, products and cases open only after verification.')}</p></section>`}
