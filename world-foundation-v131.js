(() => {
  ...
  async function openReceipt(id) {
    try { const result=await api(`/api/v1/world/receipts/${encodeURIComponent(id)}`); renderReceiptDialog(result.receipt,result.companyProductMatches); }
    catch(error){setStatus(error.message,true)}
  }

  function renderReceiptDialog(receipt,companyProductMatches=null) {
    ...
    const things=state.data?.things||[];
    const officialByItem=new Map((companyProductMatches?.items||[]).map(item=>[item.receiptItemId,item.matches||[]]));
    const itemRows=receipt.items.length?receipt.items.map(item=>receiptItemRow({...item,companyMatches:officialByItem.get(item.publicId)||[]},things)).join(''):receiptItemRow({publicId:'',title:'',quantity:1,totalCents:null,currency:receipt.currency,confidence:1,duplicateCandidates:[],companyMatches:[]},things);
  ...
  async function openThing(id){
    try{
      ...
      if(thing.officialProduct){const identitySection=$('.wv131-passport section',dialog),official=thing.officialProduct,warranty=thing.warranty,relationship=thing.companyRelationship;identitySection.insertAdjacentHTML('afterend',`<section class="wv131-official-product"><h4>${t('Official product identity','Službeni identitet proizvoda')}</h4><p><b>${esc([official.brand,official.name,official.model].filter(Boolean).join(' · '))}</b><small>${esc(official.company.name)} · ${esc(String(official.source).replaceAll('_',' '))}</small></p>${warranty?`<p><b>${esc(warranty.name)} · ${esc(warranty.status)}</b>${warranty.expiresAt?`<small>${t('Until','Do')} ${date(warranty.expiresAt)}</small>`:''}</p>`:''}${relationship?.active?`<div class="wv131-passport-actions"><button type="button" data-company-request="SUPPORT">${t('Get support','Zatraži podršku')}</button><button type="button" data-company-request="WARRANTY">${t('Warranty claim','Zahtjev za jamstvo')}</button><button type="button" data-company-request="RETURN">${t('Request return','Zatraži povrat')}</button></div><small>${t('Only product identity, purchase reference and the message you approve are shared.','Dijele se samo identitet proizvoda, referenca kupnje i poruka koju odobriš.')}</small>`:`<small>${t('No active company access. Your official product identity remains in your Passport.','Nema aktivnog pristupa tvrtke. Službeni identitet proizvoda ostaje u tvojoj Putovnici.')}</small>`}</section>`);$$('[data-company-request]',dialog).forEach(button=>button.onclick=()=>openCompanyRequest(thing,button.dataset.companyRequest))}$('[data-passport-qr]',dialog).onclick=()=>createPassportQr(id);$('[data-thing-archive]',dialog).onclick=()=>archiveThing(thing);$('[data-attach-receipt]',dialog).onclick=()=>{dialog.close();openView('capture')};$$('[data-receipt]',dialog).forEach(button=>button.onclick=()=>openReceipt(button.dataset.receipt));
    }catch(error){setStatus(error.message,true)}
  }
  async function openCompanyRequest(thing,type){const label={SUPPORT:t('Support request','Zahtjev za podršku'),WARRANTY:t('Warranty claim','Zahtjev za jamstvo'),RETURN:t('Return request','Zahtjev za povrat')}[type],description=prompt(`${label}\n\n${t('Describe what you need. Still will share only product identity, purchase reference and this message.','Opiši što trebaš. Still će podijeliti samo identitet proizvoda, referencu kupnje i ovu poruku.')}`);if(!description?.trim())return;try{const result=await api('/api/v1/company-network/support-requests',{method:'POST',body:JSON.stringify({thingId:thing.publicId,type,subject:label,description:description.trim(),sharedFields:['product_identity','purchase_reference','buyer_message'],evidence:[]})});setStatus(`${label} · ${result.requestId}`);dialog.close()}catch(error){setStatus(error.message,true)}}
