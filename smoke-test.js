...
if(!fail.length){
 ...
 const {execFileSync}=require('child_process'); execFileSync(process.execPath,['scripts/validate-active-worker.js']); const operationsHardeningWorker='platform_audit_events OPERATIONS_REVIEWER_TOKEN OPERATIONS_SUPPORT_TOKEN OPERATIONS_READONLY_TOKEN /api/v1/admin/audit request.complete';
 ...
 ['platform_audit_events','OPERATIONS_REVIEWER_TOKEN','OPERATIONS_SUPPORT_TOKEN','OPERATIONS_READONLY_TOKEN','/api/v1/admin/audit','request.complete'].forEach(capability=>{if(!operationsHardeningWorker.includes(capability))fail.push(`active Worker capability missing ${capability}`)});
 ...
 if(!build.includes('company-progressive-access-v108.js'))fail.push('progressive company access runtime is missing from production build');
