...
fs.copyFileSync(path.join(__dirname,'_headers'),path.join(out,'_headers'));fs.writeFileSync(path.join(out,'build.json'),JSON.stringify({app:'Still',productionBundle:BUNDLE,architecture:'still-os-world-needs-resolution-ownership-native-market-with-preserved-company-workspace',generatedAt:new Date().toISOString(),runtime,authenticatedApp:['app.html','still-os-v133.js','still-os-v133.css','needs-resolution-v134.js','needs-resolution-v134.css','still-market-v135.js','still-market-v135.css'],companyScripts,companyFeatureScripts,files},null,2)+'\n');console.log(`Built V${BUNDLE}: Still OS with persisted Needs, real resolution and ownership-native Market.`);
