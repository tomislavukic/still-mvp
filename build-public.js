const fs=require('fs'),path=require('path');const BUNDLE=161;
...
function extractStyleBlocks(html){const lower=html.toLowerCase(),chunks=[],styles=[];let cursor=0;while(cursor<html.length){const open=lower.indexOf('<style',cursor);if(open<0){chunks.push(html.slice(cursor));break}const tagEnd=lower.indexOf('>',open+6),close=tagEnd<0?-1:lower.indexOf('</style>',tagEnd+1);if(tagEnd<0||close<0)throw Error('Malformed inline style block in canonical index.html');chunks.push(html.slice(cursor,open));const source=html.slice(tagEnd+1,close).trim();if(source)styles.push(source);cursor=close+8}return{html:chunks.join(''),styles}}
const ip=path.join(out,'index.html');let h=fs.readFileSync(ip,'utf8');const extractedStyles=extractStyleBlocks(h);h=extractedStyles.html;if(extractedStyles.styles.length){const sp=path.join(out,'styles.css');fs.writeFileSync(sp,fs.readFileSync(sp,'utf8')+'\n'+extractedStyles.styles.join('\n'))}h=stripStyle(h,'share-icons.css');h=h.replace(/<link rel=["']stylesheet["'] href=["']styles\.css["']\s*\/?>/i,m=>m+'\n'+publicStyles.map(f=>`<link rel="stylesheet" href="${f}?v=${BUNDLE}">`).join('\n'));for(const f of runtime)h=stripScript(h,f);for(const f of companyScripts)h=stripScript(h,f);h=h.replace(/<main(?:\s[^>]*)?>[\s\S]*?<\/main>/i,'<main></main>');const tags=publicRuntime.map(f=>`<script src="${f}?v=${BUNDLE}" defer></script>`).join('\n');const googleConnections='<link rel="preconnect" href="https://accounts.google.com"><link rel="preconnect" href="https://accounts.gstatic.com" crossorigin>';h=h.replace('</body>',tags+'\n</body>').replace(/<meta name=["']still-build["'][^>]*>/gi,'').replace('</head>',`${googleConnections}<meta name="still-build" content="${BUNDLE}"></head>`);const marker=`<span id="stillBuildMarker" style="display:block;margin-top:8px;font-size:9px;font-weight:500;opacity:.55">Build ${BUNDLE}</span>`;h=/<footer[\s>]/i.test(h)?h.replace(/(<\/footer>)/i,marker+'$1'):h.replace('</body>',marker+'</body>');fs.writeFileSync(ip,h);
