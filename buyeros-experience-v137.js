(() => {
  ...
  function installStyles() {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      ${ROOT_SELECTOR} .bos132-main > *{
        animation:bos137-enter .32s cubic-bezier(.22,.72,.18,1) both
      }

      @keyframes bos137-enter{
        from{opacity:0;transform:translateY(7px)}
        to{opacity:1;transform:none}
      }

      ${ROOT_SELECTOR} .bos132-card,
      ${ROOT_SELECTOR} .bos132-section,
      ${ROOT_SELECTOR} .bos132-thing-card,
      ${ROOT_SELECTOR} .bos135-hero,
      ${ROOT_SELECTOR} .bos135-metric,
      ${ROOT_SELECTOR} .bos136-status,
      ${ROOT_SELECTOR} .bos136-next,
      ${ROOT_SELECTOR} .bos136-knowledge-row{
        transition:transform .2s ease,border-color .2s ease,background-color .2s ease,box-shadow .2s ease
      }

      ${ROOT_SELECTOR} .bos132-card:hover,
      ${ROOT_SELECTOR} .bos135-metric:hover,
      ${ROOT_SELECTOR} .bos136-status:hover,
      ${ROOT_SELECTOR} .bos136-knowledge-row:hover{
        transform:translateY(-2px);
        border-color:color-mix(in srgb,var(--line,#d9e1e5) 65%,var(--ink,#111))
      }
      ...
      ${ROOT_SELECTOR} .bos132-thing-card::after{
        content:'';
        position:absolute;
        inset:0;
        pointer-events:none;
      ...
      ${ROOT_SELECTOR} .bos132-thing-card:hover{
      ...
      ${ROOT_SELECTOR} .bos132-nav button.active::before{
        content:'';
        position:absolute;
        left:0;
        top:50%;
      ...
      ${ROOT_SELECTOR} .bos136-health-bar i{
        transform-origin:left center;
        animation:bos137-health .6s cubic-bezier(.2,.75,.15,1) both
      }

      @keyframes bos137-health{
      ...
      ${ROOT_SELECTOR} .bos135-tabs button::after{
        content:'';
        position:absolute;
        left:11px;
        right:11px;
      ...
      ${ROOT_SELECTOR} .bos135-tabs button[data-bos137-active='true']{
      ...
      @keyframes bos137-focus{
        0%{box-shadow:0 0 0 0 rgba(99,91,255,0)}
        35%{box-shadow:0 0 0 4px rgba(99,91,255,.14)}
      ...
      ${ROOT_SELECTOR} .bos137-empty-action{
        display:flex;
      ...
      @media(prefers-reduced-motion:reduce){
        ${ROOT_SELECTOR} .bos132-main > *,
        ${ROOT_SELECTOR} .bos136-health-bar i,
        ${ROOT_SELECTOR} .bos136-status-strip > *{animation:none!important}
        ${ROOT_SELECTOR} *{scroll-behavior:auto!important}
        ${ROOT_SELECTOR} .bos132-card,
        ${ROOT_SELECTOR} .bos132-section,
        ${ROOT_SELECTOR} .bos132-thing-card,
        ${ROOT_SELECTOR} .bos135-hero,
        ...
        ${ROOT_SELECTOR} .bos136-next,
        ${ROOT_SELECTOR} .bos136-knowledge-row,
        ${ROOT_SELECTOR} button{transition:none!important}
        ${ROOT_SELECTOR} button:hover,
        ${ROOT_SELECTOR} .bos132-card:hover,
        ${ROOT_SELECTOR} .bos132-thing-card:hover,
        ${ROOT_SELECTOR} .bos135-metric:hover,
        ${ROOT_SELECTOR} .bos136-status:hover,
        ${ROOT_SELECTOR} .bos136-knowledge-row:hover{transform:none!important}
      }
    `;
  ...
  function decorateEmptyStates(root) {
    root.querySelectorAll('.bos132-empty').forEach(empty => {
      ...
      const clearText = ['nothing urgent', 'ništa hitno', 'ništa hitno', 'no urgent'];
      if (!clearText.some(token => text.includes(token))) return;
  ...
  function decoratePassport(root) {
    ...
    const identity = hero.querySelector('.bos136-identity > div:last-child');
    if (identity) identity.appendChild(kicker);
  }
  ...
  function bindClickFeedback(root) {
    ...
    root.addEventListener('click', event => {
      ...
      if (openThing) {
        focusSection(openThing.closest('.bos132-thing-card'));
      }
    });
  }
  ...
  function syncPassportTabs(root) {
    ...
    tabObserver = new IntersectionObserver(entries => {
      ...
      tabs.forEach(tab => {
        tab.dataset.bos137Active = String(tab.dataset.bos135Jump === id);
      });
    }, {
